...
/**
 * TODO: build the add-on with the Palemoon SDK.
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
...
const { setDoNotSortAnnotation, setRecursiveAnnotation } = require('annotations');
...
function adjustSortCriteria() {
    ...
    bookmarkSorter.setCriteria(sortCriterias[prefs.sort_by], prefs.inverse, sortCriterias[prefs.then_sort_by], prefs.then_inverse, sortCriterias[prefs.folder_sort_by], prefs.folder_inverse, differentFolderOrder, prefs.case_insensitive);
    sortIfAuto();
}
    ...
    if(upgrade) {
        ...
        if(prefs.migration < '2.8') {
            ...
            if(prefs.sort_menu === false) {
                setDoNotSortAnnotation(MENU.id);
                setRecursiveAnnotation(MENU.id);
            }

            if(prefs.sort_toolbar === false) {
                setDoNotSortAnnotation(TOOLBAR.id);
                setRecursiveAnnotation(TOOLBAR.id);
            }

            if(prefs.sort_unsorted === false) {
                setDoNotSortAnnotation(UNSORTED.id);
                setRecursiveAnnotation(UNSORTED.id);
            }
