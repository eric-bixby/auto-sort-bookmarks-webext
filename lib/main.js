...
const _ = require('sdk/l10n').get,
    ...
    { BookmarkManager, Folder, Separator, getRootFolders } = require('bookmarks'),
    bookmarkManager = new BookmarkManager({}),
    { BookmarkSorter } = require('bookmark-sorter'),
    { getOptionName, setHiddenPreference, setPreferenceMaximum, setPreferenceMinimum } = require('options'),
...
function createEvents() {
    ...
    simplePrefs.on('exclude_folders', showConfigureFoldersToExclude);
    ...
    if(upgrade) {
        ...
        if(!prefs.folders_to_exclude) {
            ...
            if(prefs.sort_menu === false) {
                foldersToExclude.push(MENU.id);
            }
            if(prefs.sort_toolbar === false) {
                foldersToExclude.push(TOOLBAR.id);
            }
            if(prefs.sort_unsorted === false) {
                foldersToExclude.push(UNSORTED.id);
            }
            prefs.folders_to_exclude = JSON.stringify(foldersToExclude);
        }
    }
...
function showConfigureFoldersToExclude() {
    function addFolderToExclude(folderID) {
        let foldersToExclude = JSON.parse(prefs.folders_to_exclude);
        foldersToExclude.push(folderID);
        prefs.folders_to_exclude = JSON.stringify(foldersToExclude);
    }
    ...
    function removeFolderToExclude(folderID) {
        let foldersToExclude = JSON.parse(prefs.folders_to_exclude);
        arrayRemove(foldersToExclude, folderID);
        prefs.folders_to_exclude = JSON.stringify(foldersToExclude);
    }

    function updateFoldersToExclude(folderID, activated) {
        if(activated) {
            removeFolderToExclude(folderID);
        }
        else {
            addFolderToExclude(folderID);
        }
        sortIfAuto();
    }
    ...
    tabs.open({
        ...
        onOpen: function(tab) {
            tab.on('ready', function() {
                ...
                worker.port.emit('folders', getRootFolders(), JSON.parse(prefs.folders_to_exclude));
            });
        }
    });
}
