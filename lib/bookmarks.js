    ...
    createObserver: function() {
        ...
        let bookmarkObserver = {
            ...
            onItemChanged: function(itemID, property, isAnnotationProperty, newValue, lastModified, itemType, parentID) {
                emit(self, 'change', createItem(itemType, itemID, undefined, parentID), property === '', property === 'bookmarkPropertiesDialog/folderLastUsed');
            },
...
function getFolderToExcludeById(id) {
    let foldersToExclude = JSON.parse(prefs.folders_to_exclude);
    let folderToExclude = null;
    for(let folder of foldersToExclude) {
        if(folder.id === id) {
...
let Folder = new Class({
    ...
    canBeSorted: function() {
        let folderToExclude = getFolderToExcludeById(this.id);
        if(folderToExclude !== null || this.hasAncestorExcluded()) {
            return false;
        }
    ...
    getFolders: function*() {
        ...
        for(let i = 0 ; i < rootNode.childCount ; ++i) {
            ...
            let folderToExclude = getFolderToExcludeById(node.itemId);
            if(folderToExclude === null || !folderToExclude.recursive) {
...
exports.getFolderToExcludeById = getFolderToExcludeById;
