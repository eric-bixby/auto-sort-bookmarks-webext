...
let Item = new Class({
    ...
    /**
     * Check if this item has an `ancestor`.
     * @param {int} ancestor The ancestor.
     * @return {boolean} Whether this item as an ancestor or not.
     */
    hasAncestor: function(ancestor) {
        if(ancestor.id === this.id) {
            ...
            if(parentID > 0) {
                ...
                return parentFolder.hasAncestor(ancestor);
            }
        }
        return false;
    },
    ...
    saveIndex: function() {
        try {
            ...
            console.log('failed to move', this.id, '.', this.title, 'to', this.index, '(', this.url, ')');
        }
    },
    /**
    ...
    setIndex: function(index) {
        this.oldIndex = this.index !== undefined ? this.index : index;
        this.index = index;
    }
});
...
let Bookmark = new Class({
    ...
    initialize: function(itemID, index, parentID, title, dateAdded, lastModified, url, lastVisited, accessCount) {
        ...
        this.order = prefs.bookmark_sort_order;
...
let Folder = new Class({
    ...
    canBeSorted: function() {
        if(!prefs.sort_menu && this.hasAncestor(menuFolder)) {
            return false;
        }
        
        if(!prefs.sort_toolbar && this.hasAncestor(toolbarFolder)) {
            return false;
        }
        
        if(!prefs.sort_unsorted && this.hasAncestor(unsortedFolder)) {
            return false;
        }
        return !(this.isReadOnly() || this.isRoot());
    },
    extends: Bookmark,
    /**
    ...
    initialize: function(itemID, index, parentID, title, dateAdded, lastModified) {
        ...
        this.order = prefs.folder_sort_order;
    },
    /**
     * Check if the folder is read-only.
     * @return {boolean} Whether the folder is read-only.
     */
    isReadOnly: function() {
        return bookmarkService.getFolderReadonly(this.id);
    },
    /**
...
let Livemark = new Class({
    ...
    initialize: function(itemID, index, parentID, title, dateAdded, lastModified) {
        Bookmark.prototype.initialize.call(this, itemID, index, parentID, title, dateAdded, lastModified);
        this.order = prefs.livemark_sort_order;
    }
});
...
let SmartBookmark = new Class({
    ...
    initialize: function(itemID, index, parentID, title) {
        ...
        this.order = prefs.smart_bookmark_sort_order;
    }
});
