...
const { Folder, getFolderToExcludeById, menuFolder, toolbarFolder, unsortedFolder } = require('bookmarks');
...
let BookmarkSorter = new Class({
    ...
    compare: function(bookmark1, bookmark2) {
        ...
        if(BookmarkSorter.prototype.differentFolderOrder && bookmark1 instanceof Folder && bookmark2 instanceof Folder) {
            ...
            if(BookmarkSorter.prototype.firstSortCriteria === 'revurl') {
    ...
    getAllFolders: function*() {
        ...
        let folder = getFolderToExcludeById(menuFolder.id);
        if(folder === null || !folder.recursive) {
            folders.push(menuFolder);
            yield menuFolder;
        }

        folder = getFolderToExcludeById(toolbarFolder.id);
        if(folder === null || !folder.recursive) {
            folders.push(toolbarFolder);
            yield toolbarFolder;
        }

        folder = getFolderToExcludeById(unsortedFolder.id);
        if(folder === null || !folder.recursive) {
