...
let BookmarkSorter = new Class({
    ...
    sortAllBookmarks: function () {
        let t0 = getStartTime("sortAllBookmarks");

        let p1 = new Promise((resolve, reject) => {
            let t1 = getStartTime("sortAllBookmarks:promise1");
            let folders = [];
            ...
            getFinishTime("sortAllBookmarks:promise1", t1);
            resolve(folders);
        });

        let p2 = new Promise((resolve, reject) => {
            let t2 = getStartTime("sortAllBookmarks:promise2");
            let folders = [];
            ...
            getFinishTime("sortAllBookmarks:promise2", t2);
            resolve(folders);
        });

        let p3 = new Promise((resolve, reject) => {
            let t3 = getStartTime("sortAllBookmarks:promise3");
            let folders = [];
            ...
            getFinishTime("sortAllBookmarks:promise3", t3);
            resolve(folders);
        });

        Promise.all([p1, p2, p3]).then(folders => {
            let t4 = getStartTime("sortAllBookmarks:promiseAll");
            // Flatten array of arrays into array
            let merged = [].concat.apply([], folders);
            this.sortFolders(merged);
            getFinishTime("sortAllBookmarks:promiseAll", t4);
        }, reason => {
            console.log(reason);
        });

        getFinishTime("sortAllBookmarks", t0);
    },
    ...
    sortAndSave: function (folder) {
        let t0 = getStartTime("sortAndSave");
    ...
    sortFolders: function (folders) {
        let t0 = getStartTime("sortFolders");
        ...
        Promise.all(promiseAry).then(bool => {
            //console.log("sortFolders:promiseAll (done)");
            self.sorting = false;
            self.changed = false;
        }, reason => {
            console.log(reason);
