...
let BookmarkSorter = new Class({
    ...
    sortAllBookmarks: function () {
        ...
        //this.sortFolders(this.getAllFolders());

        // let self = this;
        // bookmarkService.runInBatchMode({
        //     runBatched(param) {
        //         self.sortFolders(self.getAllFolders());
        //     },
        // }, null);

        let self = this;
        var myWorker = new BasePromiseWorker('resource://sortbookmarks-at-bouanto/data/worker.js');
        var promise = myWorker.post('sortAll', [this.getAllFoldersChildren()]);
        promise.then(
            function (folders) {
                bookmarkService.runInBatchMode({
                    runBatched(param) {
                        self.saveAll(folders);
                    },
                }, null);
            },
            function (aReason) {
                console.error('Rejected - ', aReason);
            }
        ).catch(
            function (aCaught) {
                console.error('Caught - ', aCaught);
            }
        );
    ...
    /**
     * Get all children for all folders.
     */
    getAllFoldersChildren: function () {
    ...
    /**
     * Save all sorted folders and children.
     */
    saveAll: function (folders) {
        console.log("saveAll::begin");
        var start = new Date().getTime();
        for (let folder of folders) {
            for (let i = 0; i < folder.children.length; ++i) {
                let length = folder.children[i].length;
                for (let j = 0; j < length; ++j) {
                    let id = folder.children[i][j].id;
                    let index = folder.children[i][j].index;
                    let oldIndex = folder.children[i][j].oldIndex;
                    if (index != oldIndex) {
                        this.setIndex(id, index);
                    }
                }
            }
        }
        var end = new Date().getTime();
        var time = end - start;
        console.log("saveAll::end,elapsed=" + time + "ms");
    },

    /**
     * Set new index of sorted item.
     */
    setIndex: function (id, index) {
        try {
            bookmarkService.setItemIndex(id, index);
        }
        catch (exception) {
            console.error("failed to move " + id);
        }
    },
